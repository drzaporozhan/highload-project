...
1. **Разделяем “витрину” и “транзакции”**: каталог/поиск — read-heavy (кэш + индекс), заказы/оплата — отдельный транзакционный контур.
2. **Поиск — отдельная подсистема**: индексация товаров и обновления цен/остатков идут асинхронно через очередь событий.
3. **Заказ как state machine**: статусы заказа меняются событиями, на них подписаны уведомления/логистика/аналитика.
4. **Идемпотентность**: ключевые операции (создание заказа/платёж) защищены idempotency key и ретраями.
5. **Медиа отдельно**: фото/документы — объектное хранилище + CDN.
6. **Защита**: rate limiting и базовый антифрод.
