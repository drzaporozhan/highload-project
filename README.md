...
|----------------------------|-------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Инвертированный индекс     | Хранение и быстрый поиск по каталогу      | Для каждого терма хранится список документов, в которых он встречается. За счёт этого поиск выполняется без полного сканирования каталога. [[1]](https://docs.opensearch.org/latest/getting-started/intro/)                                                                                                                                                                            |
| Анализ текста (`analyzer`) | Индексация и обработка поискового запроса | При индексации и поиске текст приводится к единому виду: разбивается на токены и нормализуется. Это нужно, чтобы запрос и данные в индексе сравнивались в одинаковом формате. [[2]](https://docs.opensearch.org/latest/analyzers/)                                                                                                                                                     |
| BM25                       | Базовое ранжирование результатов          | BM25 рассчитывает базовую текстовую релевантность `_score`. Алгоритм повышает оценку карточек, где термы запроса встречаются чаще и являются более редкими в коллекции, и снижает вклад слишком длинных текстов. [[3]](https://docs.opensearch.org/latest/search-plugins/keyword-search/)[[6]](https://www.elastic.co/blog/practical-bm25-part-2-the-bm25-algorithm-and-its-variables) |
| Вес полей                  | Приоритет полей при поиске                | После расчёта BM25 применяется фиксированный приоритет полей: `title = 4.0`, `brand = 2.5`, `attributes = 1.5`, `searchable_text = 1.0`. За счёт этого совпадение в `title` влияет на позицию сильнее, чем совпадение в остальных полях. [[4]](https://docs.opensearch.org/latest/query-dsl/compound/index/)                                                                           |
| `bool` + `filter`          | Фильтрация выдачи                         | Используется для обязательных ограничений: `status = active`, `availability = true`, диапазон цены, категория, срок доставки. Фильтры не увеличивают `_score`, а только исключают неподходящие карточки из выдачи. [[4]](https://docs.opensearch.org/latest/query-dsl/compound/index/)                                                                                                 |
| `function_score`           | Дополнительная корректировка              | После текстового ранжирования применяется пересчёт с учётом `rating`, `popularity` и `delivery_days`. Итоговый порядок задаётся формулой `score_final = score_bm25 * weight_field * k_rating * k_popularity * k_delivery`. [[5]](https://docs.opensearch.org/latest/query-dsl/compound/function-score/)                                                                                |
