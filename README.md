...
- `media_binary_objects` — S3-совместимое объектное хранилище;
...
| `media_binary_objects` | файловые данные            | объектное хранение                            | S3-совместимое объектное хранилище, HDD | нет на уровне приложения    | пока карточка активна + 180 дней после архивирования                             | оригиналы и производные изображения       |
...
| Кэш             | Технология | Где используется  | Политика                         | Описание                             |
|-----------------|------------|-------------------|----------------------------------|--------------------------------------|
| `search_cache`  | Redis      | поиск             | 1 минута                         | кэш горячих поисковых запросов       |
| `product_cache` | Redis      | просмотр карточки | 5 минут, удаление при обновлении | кэш готовой карточки товара          |
| `order_drafts`  | Redis      | корзина, checkout | 30 дней                          | корзина и черновик оформления заказа |
...
| S3-совместимое объектное хранилище | S3 SDK                              | загрузка и чтение файлов       |
...
| объектное хранилище     | загрузка через S3 API                 | чтение через CDN и S3 gateway                              | раздача файлов вынесена из OLTP-контура                 | хранение и выдача изображений                               |
...
| S3-совместимое объектное хранилище (`media_binary_objects`)     | версионирование объектов                                                       | постоянно                                                                        | версии удалённых и изменённых объектов — 30 дней                                        | восстановление файла из истории версий                                                                                                  |
